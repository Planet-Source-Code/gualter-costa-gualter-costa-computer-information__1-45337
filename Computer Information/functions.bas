...
SubKeys = GetAllKeys(lhRemoteRegistry, "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall")

'A = RegOpenKey(hKey, SubKeys, hCurKey)
...
If VarType(SubKeys) = vbArray + vbString Then
    For KeyLoop = 0 To UBound(SubKeys)
       ...
       ' sDispName = GetSettingString(HKEY_LOCAL_MACHINE, "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\" & SubKeys(KeyLoop), "DisplayName")
        ...
        sDispName = GetSettingString(lhRemoteRegistry, "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\" & SubKeys(KeyLoop), "DisplayName")
        
        If Err.Number > 0 Then Exit For
        'Debug.Print sDispName
        If sDispName > "" And Left(SubKeys(KeyLoop), 1) <> "{" Then
            main.List1.AddItem sDispName & " / " & SubKeys(KeyLoop)
            MyData = sDispName & " / " & SubKeys(KeyLoop)
            'CurrentDb.Execute "INSERT INTO tblList ( Info ) SELECT '" & MyData & "' AS MyData;"
            DoEvents
        End If
    Next
End If
